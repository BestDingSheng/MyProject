/*
 * File: app/model/Role.js
 *
 * This file was generated by Sencha Architect version 3.1.0.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 5.0.x library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 5.0.x. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('Evcorp.model.CarStock', {
    extend: 'Evcorp.model.Base',

    requires: [
        'Ext.data.field.Field'
    ],
    identifier: 'negative',
    fields: [
		{
		    name:'carStockId'
		},
		{
		    name:'carAssetId'
		},
        {
            name:'vin'
        },
        {
            name:'stock'
        },
        {
            name:'stockPositionNo'
        },
        {
            name: 'stockStatus'
        },
        {
        	name:'stockInType'
        },
        {
            name: 'stockOutType'
        },
        {
            name: 'damageStatus'
        },
        {
        	name:'distributionStatus'
        },
        {
            name:'agency'
        },
        {
            name:'stockInDate'
        },
        {
        	name:'stockOutDate'
        },
        {
        	name:'isTestDrive'
        },
        {
            name:'isUserdCar'
        },
        {
            name:'isSalesPromotion'
        },
        {
            name: 'isBackCar'
        },
        {
            name: 'isBigCustomer'
        },
        {
        	name:'isEnterInAccount'
        },
        {
            name:'isAgentAcceptance'
        },
        {
            name:'isDirectSelling'
        },
        {
        	name:'acceptanceResult'
        },
        {
        	name:'acceptanceRemark'
        },
        {
            name:'inspector'
        },
        {
            name:'inspectionDate'
        },
        {
            name: 'saleOrderNo'
        },
        {
            name: 'firstStockInDate'
        },
        {
        	name:'lastStockInDate'
        },
        {
            name:'lastStockInPerson'
        },
        {
            name:'firstStockOutDate'
        },
        {
        	name:'lastStockOutDate'
        },
        {
        	name:'lastStockOutPerson'
        },
        {
        	name:'isLocked'
        },
        {
        	name:'carProductCode'
        },
        {
        	name:'carModelCode'
        },
        {
        	name:'carBrandCode'
        },
        {
        	name:'carSeriesCode'
        },
        {
            name:'carColorCode'
        },
        {
            name:'carInteriorCode'
        },
        {
        	name:'carProductName'
        },
        {
        	name:'carModelName'
        },
        {
        	name:'carBrandName'
        },
        {
        	name:'carSeriesName'
        },
        {
            name:'carColorName'
        },
        {
            name:'carInteriorName'
        },
        {
            name:'carEngineNo'
        },
        {
            name:'carDriveLicenseNo'
        },
        {
            name:'carKeyNo'
        },
        {
            name:'displacement'
        },
        {
            name:'dischargeStandard'
        },
        {
            name:'madeDate'
        },
        {
            name:'productionDate'
        },
        {
            name:'taxPurchasePrice'
        },
        {
            name:'taxRate'
        },
        {
            name:'taxExcludedPurchasePrice'
        },
        {
            name:'oemDirectivePrice'
        },
        {
            name:'directivePrice'
        },
        {
            name:'carType'
        },
        {
            name:'sourceOfFund'
        },
        {
            name:'carFreePeriod'
        },
        {
            name:'additionalCost'
        },
        {
            name:'carOnTheWayDay'
        },
        {
            name:'sendCarDate'
        },
        {
            name:'expectedDeliveryDate'
        },
        {
            name:'realDeliveryDate'
        },
        {
            name:'invoiceNo'
        },
        {
            name:'deliveryManName'
        },
        {
            name:'deliveryManPhone'
        },
        {
            name:'deliveryCarPlateNo'
        },
        {
            name:'deliveryCompanyName'
        },
        {
            name:'purchaseDate'
        },
        {
            name:'receiptAddress'
        },
        {
            name:'venderCode'
        },
        {
            name:'venderName'
        },
        {
            name:'productingAddress'
        },
        {
            name:'certificateLocated'
        },
        {
            name:'carCallNo'
        },
        {
            name:'supplierOrderNo'
        },
        {
            name:'lastStockInDateStart'
        },
        {
            name:'lastStockInDateEnd'
        },
        {
            name:'lastStockOutDateStart'
        },
        {
            name:'lastStockOutDateEnd'
        },
        {
            name:'productionDateStart'
        },
        {
            name:'productionDateEnd'
        },
        {
            name:'remark'
        },
        {
            name:'lastStockInRemark'
        },
        {
            name:'lastStockOutRemark'
        },
    ],
    proxy: {
        type: 'direct',
        api: {
            read: carStockContro.findCarStockList,
//            create: contractCarOrderContro.createContractCarOrder,
//            update: contractCarOrderContro.updateContractCarOrder
        },
        reader: {
            type: 'json',
            rootProperty: 'records',
            totalProperty: 'total'
        }
    }
});