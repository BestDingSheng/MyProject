    ...
    SelectList.prototype={
        ...
        renderSelect: function(element){
            var that = this,
                ...
                selectLength = that.getOptionCount($this),
                selectHTML = '<div id="' + selectID + '" class="select-wrapper ' + selectClassName + '"><input type="hidden" name="' + selectName + '" value="' + selectedValue + '" /><i class="icon select-down"></i><input type="button" id="cityPosition" class="select-button" value="' + selectedOptionText + '" /><div class="select-list"><ul>',
                selectListHTML = '',
                selectListLiHTML = '<li data-value="';

                
            for(var i=0; i<selectLength; i++){
                if(i !== selectedIndex){
                    selectListHTML = selectListHTML + selectListLiHTML + selectOptionValue[i] + '">' + selectOptionText[i] + '</li>';
                }else{
                    selectListHTML = selectListHTML + selectListLiHTML + selectOptionValue[i] + '" class="selected">' + selectOptionText[i] + '</li>';
                }
